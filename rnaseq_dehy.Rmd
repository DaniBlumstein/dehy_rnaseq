...
theme<-theme(panel.background = element_blank(),panel.border=element_blank(),axis.line = element_line(colour = "black"),panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
             strip.background=element_blank(),axis.text.x=element_text(colour="black",size=16),axis.text.y=element_text(colour="black",size=8),
...
go_wgcna_lu <- go_wgcna_lu %>% rename(GO = V1,description = V2,lu_trt = trt,lu_body.temp.1=body_temp_3.x, lu_body.temp.2=body_temp_3.y, lu_wlr.1=mean_H2Og.x,lu_body.temp.3=body_temp_3.x.x,lu_body.temp.4= body_temp_3.y.y,lu_TCO2=TCO2,lu_wlr.2=mean_H2Og.y,lu_iCa = iCa, lu_body.temp.5=body_temp_3,lu_EE=mean_EE)

go_wgcna_liv <- dplyr::filter(go_wgcna_liv, grepl('GO', V1))
go_wgcna_liv <- go_wgcna_liv %>% rename(GO = V1,description = V2, liv_body.temp.1=body_temp_3.x, liv_body.temp.2=body_temp_3.y,liv_body.temp.3=body_temp_3.x.x,liv_body.temp.4=body_temp_3.y.y,liv_TCO2.1=TCO2.x,liv_iCa=iCa,liv_body.temp.5=body_temp_3.x.x.x,liv_body.temp.6=body_temp_3.y.y.y,liv_EE.1=mean_EE.x,liv_body.temp.7=body_temp_3.x.x.x.x,liv_wlr.1=mean_H2Og.x,liv_trt=trt,liv_body.temp.8=body_temp_3.y.y.y.y,liv_TCO2.2=TCO2, liv_EE.2=mean_EE.y,liv_wlr.2=mean_H2Og.y,liv_TCO2.3= TCO2.y)

go_wgcna_gi <- dplyr::filter(go_wgcna_gi, grepl('GO', V1))
go_wgcna_gi <- go_wgcna_gi %>% rename(GO = V1,description = V2,gi_Cl=Cl,gi_trt.1 = trt.x,gi_Crea=Crea,gi_Hb.1=Hb..x,gi_trt.2=trt.y,gi_body.temp.1=body_temp_3.x,gi_trt.3=trt.x.x,gi_wlr.1=mean_H2Og.x,gi_trt.4=trt.y.y,gi_Hb.2=Hb..y,gi_wlr.2=mean_H2Og.y,gi_wlr.3=mean_H2Og,gi_body.temp.2=body_temp_3.y,gi_Hb.3=Hb.,gi_trt.5=trt,gi_body.temp.3=body_temp_3)

go_wgcna_hyp <- dplyr::filter(go_wgcna_hyp, grepl('GO', V1))
go_wgcna_hyp <- go_wgcna_hyp %>% rename(GO = V1,description = V2,hyp_Crea.1=Crea.x,hyp_TCO2=TCO2,hyp_trt = trt,hyp_body.temp=body_temp_3,hyp_Crea.2=Crea.y,hyp_EE =mean_EE)

go_wgcna_kid <- dplyr::filter(go_wgcna_kid, grepl('GO', V1))
go_wgcna_kid <- go_wgcna_kid %>% rename(GO = V1,description = V2,kid_trt.1 =trt.x,kid_wlr.1 =mean_H2Og.x,kid_trt.2 =trt.y,kid_Cl.1 =Cl.x,kid_body.temp.1 =body_temp_3.x,kid_wlr.2 =mean_H2Og.y ,kid_K.1 =K.x,kid_Hb =Hb.,kid_wlr.3 =mean_H2Og.x.x,kid_wlr.4 =mean_H2Og.y.y,kid_wlr.5 =mean_H2Og,kid_Glu.1 =Glu.x,kid_body.temp.2 =body_temp_3.y,kid_body.temp.3 =body_temp_3,kid_Cl.2 =Cl.y,kid_TCO2 =TCO2,kid_RQ =mean_RQ,kid_Glu.2 =Glu.y,kid_K.2 =K.y)
...
go_wgcna <- go_wgcna %>% rename(count = value)
...
hyp <- ggplot(go_wgcna_clean_hyp, aes(x = variable$X2, y = description, color = variable$X2, size = count)) +
  geom_point()+
  theme+
  ggtitle("wgcna go")+
  theme(legend.position = "none")

liv <- ggplot(go_wgcna_clean_liv, aes(x = variable$X2, y = description, color = variable$X2, size = count)) +
  geom_point()+
  theme+
  ggtitle("wgcna go")+
  theme(legend.position = "none")

lu <- ggplot(go_wgcna_clean_lu, aes(x = variable$X2, y = description, color = variable$X2, size = count)) +
  geom_point()+
  theme+
  ggtitle("wgcna go")+
  theme(legend.position = "none")

gi <- ggplot(go_wgcna_clean_gi, aes(x = variable$X2, y = description, color = variable$X2, size = count)) +
  geom_point()+
  theme+
  ggtitle("wgcna go")+
  theme(legend.position = "none")

kid <- ggplot(go_wgcna_clean_kid, aes(x = variable$X2, y = description, color = variable$X2, size = count)) +
  geom_point()+
  theme+
  ggtitle("wgcna go")+
  theme(legend.position = "none")
...
ggplot(big_data,  aes(x = trt, y = log(count), fill = tissue, alpha=trt)) + 
      #ggtitle(title)  + 
      theme +
      ylab("log transofrmed Normalized Counts") + 
      xlab("Tissue") +
      geom_boxplot(outlier.shape = NA, lwd=.75) + 
      geom_point(aes(color=tissue),position = position_jitterdodge()) +
      facet_grid(gene~tissue , scales = "free") +
      theme_classic() +
      theme(panel.border = element_rect(color = "black", fill = NA, size = .5),  legend.title=element_text(size=18), legend.text=element_text(size=18),axis.text.x=element_text(colour="black",size=10),axis.text.y=element_text(colour="black",size=10))+
  scale_fill_manual(values=natparks.pals("BryceCanyon",5))+
  scale_color_manual(values=natparks.pals("BryceCanyon",5))
