...
t_subset_all_Expr <- t(subset_all_Expr)
samples<-as.data.frame(samples)
traitData_V2 <- samples %>% dplyr::select(c("trt","pro_weight","mean_RQ","mean_EE","mean_H2Og","body_temp_3","sample","tissue", "sex","mouse_ID"))

traitData_V2 <- traitData_V2 |>
    mutate(tt = fct_cross(trt, sex))

#traitData_V2$trt <- as.numeric(traitData_V2$trt)
#traitData_V2$sex <- as.numeric(traitData_V2$sex)
rownames(traitData_V2) <- traitData_V2$sample
traitData_V2 <- subset(traitData_V2, select = -sample)

cca_model <- cca(t_subset_all_Expr~tt*(pro_weight+mean_RQ+mean_EE+mean_H2Og+body_temp_3 + tissue) +Condition(mouse_ID),data = traitData_V2)
unique(traitData_V2$tt)
#cca_model <- cca(t_subset_all_Expr~tissue*(pro_weight+mean_RQ+mean_EE+mean_H2Og+body_temp_3),data = traitData_V2)
...
boxplot(df_species$CCA1)
boxplot(df_species$CCA2)

out1 <- boxplot.stats(df_species$CCA1)$out
out_ind1 <- which(df_species$CCA1 %in% c(out1))

out2 <- boxplot.stats(df_species$CCA2)$out
out_ind2 <- which(df_species$CCA2 %in% c(out2))

out<-unique(sort(append(out_ind1,out_ind2)))
outliers <- df_species[out, ]
outliers$color <- "outlier"

norm <- df_species[-out, ]
norm$color <- "in bound"

graphing_data <- as.data.frame(rbind(norm,outliers))

df_sites  <- data.frame(summary(cca_model)$sites[,1:2])# get the indivisuals CC1 and CC2 scores
df_environ  <- scores(summary(cca_model)$biplot[c(4,7,16:21),1:2]) #get the physio vars CC1 and CC2 scores
