...
write.csv(kid_countdata, file="results/GOresults/kid/kid_count_data.csv")
...
SigGeneWrapper(kid_res_trt, 0.05, "all")
summary(kid_res_trt)
...
```{r}
up_kid <- filter(as.data.frame(restrt_kid), log2FoldChange > 0, padj < 0.05)
...
down_kid <- filter(as.data.frame(restrt_kid), log2FoldChange < 0, padj < 0.05)
