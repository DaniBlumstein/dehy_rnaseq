...
hyp_samples$sex <- as.factor(hyp_samples$sex...5)
...
```{r}
modTraitP_hyp_long <- reshape2::melt(modTraitP_hyp,measure.vars = c("sex","total_weight_delta","pro_weight","Na","BUN","AnGap","K","Crea","Hct","Cl","Glu","Hb.","TCO2","iCa","trt ",'mean_RQ','mean_EE','mean_H2Og','body_temp_3'))
...
subset <- filter(combo, color=="midnightblue")
phenos <- c(unique(as.character(subset$pheno)))
subset2 <- filter(subset, pheno=="TCO2")
