...
hyp <- heatmap(hyp_Expr,Rowv=NA, col=rev(brewer.pal(9,"RdBu")))
```
...
traitData_hyp <- traitData_hyp %>% select(c('sex','total_weight_delta',"pro_weight","Na","BUN","AnGap","K","Crea","Hct","Cl","Glu","Hb*","TCO2","iCa","trt","mean_RQ","mean_EE","mean_H2Og","body_temp_3"))
#traitData_hyp <- 'row.names<-'(traitData_hyp, traitData_hyp$sample)
...
```{r}
##########I'm choosing 15 for my power based on scale independence and k for hyp ###########
...
net_hyp = blockwiseModules(hyp_Expr,corType="pearson",
                          ...
                          mergeCutHeight=merhypngThresh,numericLabels=TRUE,saveTOMs=TRUE,
...
weight_g_hyp = as.data.frame(traitData_hyp$pro_weight)
names(weight_g_hyp)="weight_g_hyp"
...
geneTraitSignificance_hyp = as.data.frame(cor(hyp_Expr, weight_g_hyp, use = "p"));
GSPvalue_hyp = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance_hyp), nSampleshyp));
names(geneTraitSignificance_hyp) = paste("GS.", names(weight_g_hyp), sep="");
names(GSPvalue_hyp) = paste("p.GS.", names(weight_g_hyp), sep="");
...
# Choose columns of trait dataset to be used in downstream analyses
colnames(traitData_hyp)
traitData_hyp <- traitData_hyp %>% select(c('sex','total_weight_delta',"pro_weight","Na","BUN","AnGap","K","Crea","Hct","Cl","Glu","Hb*","TCO2","iCa","trt","mean_RQ","mean_EE","mean_H2Og","body_temp_3","sample"))

MEs_hyp<- read.csv("results/WGCNAresults/ALLSAMPLES_hyp_MEs_signed.csv", header = TRUE)
MEs_hyp$X <-  sub("X*", "", MEs_hyp$X)

pheno_ME_hyp <- merge(MEs_hyp, traitData_hyp, by.x = "X", by.y = "sample")

plot(pheno_ME_hyp$MEgreen ~ pheno_ME_hyp$trt, col = pheno_ME_hyp$trt)
```
...
MEs_hyp2 <- MEs_hyp[,-1]
modOrder = order(-abs(cor(MEs_hyp2, weight_g_hyp, use = "p")));
...
############### GO enrichment #################
library(gprofiler2)
hyp_ref <- geneInfo0_hyp$gene

hyp_wgcna_salmon <- geneInfo0_hyp$gene[which(geneInfo0_hyp$moduleColor=="salmon")]
hyp_wgcna_lightcyan <- geneInfo0_hyp$gene[which(geneInfo0_hyp$moduleColor=="lightcyan")]
hyp_wgcna_turquoise <- geneInfo0_hyp$gene[which(geneInfo0_hyp$moduleColor=="turquoise")]
hyp_wgcna_cyan <- geneInfo0_hyp$gene[which(geneInfo0_hyp$moduleColor=="cyan")]
hyp_wgcna_magenta <- geneInfo0_hyp$gene[which(geneInfo0_hyp$moduleColor=="magenta")]
hyp_wgcna_tan <- geneInfo0_hyp$gene[which(geneInfo0_hyp$moduleColor=="tan")]
hyp_wgcna_greenyellow <- geneInfo0_hyp$gene[which(geneInfo0_hyp$moduleColor=="greenyellow")]
hyp_wgcna_midnightblue <- geneInfo0_hyp$gene[which(geneInfo0_hyp$moduleColor=="midnightblue")]

hyp_wgcna_salmon_GO <- gost(hyp_wgcna_salmon, organism = "hsapiens", 
                     ordered_query = F, significant = T, exclude_iea = T,
                     correction_method = "bonferroni",
                     custom_bg = hyp_ref)
gostplot(hyp_wgcna_salmon_GO, capped = TRUE, interactive = TRUE)
sink(file = "results/WGCNAresults/hyp_wgcna_salmon_GO.csv")
sink()


hyp_wgcna_lightcyan_GO <- gost(hyp_wgcna_lightcyan, organism = "hsapiens", 
                     ordered_query = F, significant = T, exclude_iea = T,
                     correction_method = "bonferroni",
                     custom_bg = hyp_ref)
gostplot(hyp_wgcna_lightcyan_GO, capped = TRUE, interactive = TRUE)
sink(file = "results/WGCNAresults/hyp_wgcna_lightcyan_GO.csv")
sink()


hyp_wgcna_turquoise_GO <- gost(hyp_wgcna_turquoise, organism = "hsapiens", 
                     ordered_query = F, significant = T, exclude_iea = T,
                     correction_method = "bonferroni",
                     custom_bg = hyp_ref)
gostplot(hyp_wgcna_turquoise_GO, capped = TRUE, interactive = TRUE)
sink(file = "results/WGCNAresults/hyp_wgcna_turquoise_GO.csv")
sink()


hyp_wgcna_cyan_GO <- gost(hyp_wgcna_cyan, organism = "hsapiens", 
                     ordered_query = F, significant = T, exclude_iea = T,
                     correction_method = "bonferroni",
                     custom_bg = hyp_ref)
gostplot(hyp_wgcna_cyan_GO, capped = TRUE, interactive = TRUE)
sink(file = "results/WGCNAresults/hyp_wgcna_cyan_GO.csv")
sink()

hyp_wgcna_magenta_GO <- gost(hyp_wgcna_magenta, organism = "hsapiens", 
                     ordered_query = F, significant = T, exclude_iea = T,
                     correction_method = "bonferroni",
                     custom_bg = hyp_ref)
gostplot(hyp_wgcna_magenta_GO, capped = TRUE, interactive = TRUE)
sink(file = "results/WGCNAresults/hyp_wgcna_magenta_GO.csv")
sink()


hyp_wgcna_tan_GO <- gost(hyp_wgcna_tan, organism = "hsapiens", 
                     ordered_query = F, significant = T, exclude_iea = T,
                     correction_method = "bonferroni",
                     custom_bg = hyp_ref)
gostplot(hyp_wgcna_tan_GO, capped = TRUE, interactive = TRUE)
sink(file = "results/WGCNAresults/hyp_wgcna_tan_GO.csv")
sink()


hyp_wgcna_greenyellow_GO <- gost(hyp_wgcna_greenyellow, organism = "hsapiens", 
                     ordered_query = F, significant = T, exclude_iea = T,
                     correction_method = "bonferroni",
                     custom_bg = hyp_ref)
gostplot(hyp_wgcna_greenyellow_GO, capped = TRUE, interactive = TRUE)
sink(file = "results/WGCNAresults/hyp_wgcna_greenyellow_GO.csv")
sink()


hyp_wgcna_midnightblue_GO <- gost(hyp_wgcna_midnightblue, organism = "hsapiens", 
                     ordered_query = F, significant = T, exclude_iea = T,
                     correction_method = "bonferroni",
                     custom_bg = hyp_ref)
gostplot(hyp_wgcna_midnightblue_GO, capped = TRUE, interactive = TRUE)
sink(file = "results/WGCNAresults/hyp_wgcna_midnightblue_GO.csv")
sink()
