...
SigGeneWrapper(hyp_res_trt, 0.05, "all")
summary(hyp_res_trt)
...
```{r}
up_hyp <- filter(as.data.frame(restrt_hyp), log2FoldChange > 0, padj < 0.05)
...
down_hyp <- filter(as.data.frame(restrt_hyp), log2FoldChange < 0, padj < 0.05)
