...
annos <- read.csv("/Users/danielleblumstein/Documents/UNH/rnaseq/dehy_final/data/annos.txt")
...
mod merkidng
Now, using the new found measurements of dissimilarity, you can construct a cluster tree. You will also be adding a line at the height of .25. This height corresponds to a correlation of over 75%. Any branches below this line are more than 75% related, and you will thus be merkidng them!
```{r}
ME.dissimilarity = 1-cor(MEs0kid, use="complete") #Calculate eigengene dissimilarity
METree = hclust(as.dist(ME.dissimilarity), method = "average") #Clustering eigengenes 
par(mar = c(0,4,2,0)) #seting markidn sizes
...
c("Orikidnal Module", "Merged Module"),
dendroLabels = FALSE, hang = 0.03,
addGuide = TRUE, guideHang = 0.05,
main = "Gene dendrogram and module colors for orikidnal and merged modules")
```
...
layouts <- layouts[!grepl("bipartite|merge|norm|sukidyama|tree", layouts)]
...
# Choose columns of trait dataset to be used in downstream analyses
colnames(traitData_kid)
traitData_kid <- traitData_kid %>% dplyr::select(c('sex','total_weight_delta',"pro_weight","Na","BUN","AnGap","K","Crea","Hct","Cl","Glu","Hb.","TCO2","iCa","trt","mean_RQ","mean_EE","mean_H2Og","body_temp_3","sample"))
...
GO
```{r}
modTraitP_kid_long <- reshape2::melt(modTraitP_kid,measure.vars = c("sex","total_weight_delta","pro_weight","Na","BUN","AnGap","K","Crea","Hct","Cl","Glu","Hb.","TCO2","iCa","trt ",'mean_RQ','mean_EE','mean_H2Og','body_temp_3'))
...
genes_mods_filtered$gene <- genes_mods_filtered$V1
genes_mods_filtered$color <- genes_mods_filtered$V2

modTraitP_kid_long_sig$color <-modTraitP_kid_long_sig$Var1
modTraitP_kid_long_sig$pheno <-modTraitP_kid_long_sig$Var2
modTraitP_kid_long_sig$pval <- modTraitP_kid_long_sig$value
